...
class Sales_model extends CI_Model
{
    ...
    public function getCusID($delivery_id=array())
    {
        $this->db->select("erp_delivery_items.id,
                                erp_delivery_items.delivery_id,
                                erp_deliveries.customer_id,
                                erp_deliveries.customer,
                                erp_deliveries.type,
                                erp_deliveries.address,
                                erp_companies.`name` AS delivery_name");
            $this->db->from('erp_delivery_items');
            $this->db->join('erp_deliveries', 'deliveries.id = delivery_items.delivery_id', 'left');
            $this->db->join('erp_companies', 'companies.id = deliveries.delivery_by', 'left');
            $this->db->group_by('delivery_items.delivery_id');
    ...
    public function getSaleman($delivery_id=array())
    {
        $this->db->select("erp_deliveries.id,erp_deliveries.date,erp_deliveries.id,erp_companies.name as delivery_name,erp_companies.phone,
                                erp_users.username AS saleman");
            $this->db->from('erp_deliveries');
            $this->db->join('erp_users', 'users.id = deliveries.created_by', 'left');
            $this->db->join('erp_companies', 'companies.id = deliveries.delivery_by', 'left');
            $this->db->where_in('erp_deliveries.id' , $delivery_id);
        $q = $this->db->get();
        if($q->num_rows()>0){
            return $q->row();
        }
    ...
    public function getAllItemDelivery($customer_id=null,$delivery_id=null)
    {
        ...
        $this->db->select("erp_delivery_items.id,
                   ...
                   (erp_sale_order_items.real_unit_price * erp_delivery_items.quantity_received) as subtotal,( CASE WHEN erp_products.unit = 0 THEN erp_products.unit ELSE erp_units.NAME END ) AS uname");
            $this->db->from('erp_delivery_items');
            $this->db->join('erp_deliveries', 'deliveries.id = delivery_items.delivery_id', 'left');
            $this->db->join('erp_products', 'products.id = delivery_items.product_id', 'left');
            $this->db->join('erp_units', 'units.id = products.unit', 'left');
            $this->db->join('erp_sale_order_items', 'sale_order_items.id = delivery_items.item_id', 'left');
            $this->db->group_by('delivery_items.id');
            $this->db->where('erp_delivery_items.delivery_id' , $delivery_id);
            $this->db->where('erp_deliveries.customer_id' , $customer_id);
        $q = $this->db->get();
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
    ...
    public function getAllInvoiceDelivery($customer_id=null,$delivery_id=null)
    {
        //$this->erp->print_arrays($delivery_id);
        $this->db->select("erp_delivery_items.id,
                   erp_delivery_items.delivery_id,
                   erp_delivery_items.product_name,
                   erp_delivery_items.quantity_received,
                   erp_sale_items.real_unit_price,
	               (erp_sale_items.real_unit_price * erp_delivery_items.quantity_received ) AS subtotal,( CASE WHEN erp_products.unit = 0 THEN erp_products.unit ELSE erp_units.NAME END ) AS uname");
            $this->db->from('erp_delivery_items');
            $this->db->join('erp_deliveries', 'deliveries.id = delivery_items.delivery_id', 'left');
            $this->db->join('erp_products', 'products.id = delivery_items.product_id', 'left');
            $this->db->join('erp_units', 'units.id = products.unit', 'left');
            $this->db->join('erp_sale_items', 'sale_items.id = delivery_items.item_id', 'left');
            $this->db->group_by('delivery_items.id');
            $this->db->where('erp_delivery_items.delivery_id' , $delivery_id);
            $this->db->where('erp_deliveries.customer_id' , $customer_id);
        $q = $this->db->get();
    ...
    {
        $this->db
            ->select("erp_users.last_name as saleman_last,erp_users.first_name as saleman_first,sales.*, companies.name,companies.code,companies.company,companies.logo,companies.cf4,companies.phone, companies.email,
			  quotes.reference_no as quote_no, users.first_name as first_name,users.last_name as last_name,payment_term.due_day as due_day,payment_term.description as description_en,payment_term.description_kh as description_kh,
			  ...
			  FROM erp_payments WHERE erp_payments.sale_id = erp_sales.id  ) as deposit,
			   ...
			   sales.due_date,com.vat_no as customer_vat,return_sales.grand_total as total_return,return_sales.paid as return_paid")
        ...
        if ($this->db->insert('return_sales', $data)) {
            ...
            foreach ($items as $item) {
				...
				if ($item['product_type'] == 'combo') {
					...
					$this->updatePurchaseItem(NULL, $item['quantity'], $item['sale_item_id'], $item['product_id'], $item['warehouse_id'], $item['option_id'], $return_item_id);
				}
...
/*    public function getSaleByRef($ref)
	...
	public function getSaleOrder($sale_order_id){
		$this->db->select("erp_users.last_name as saleman_last,erp_users.first_name as saleman_first,sale_order.*, companies.name, companies.company,erp_payment_term.description as pt_dc,erp_users.username as saleman, 
