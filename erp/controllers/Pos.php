...
class Pos extends MY_Controller
{
    ...
    function index($sid = null,$sale_order_id=null)
    {
        ...
        $this->form_validation->set_rules('reference_nob', $this->lang->line("reference_nob"), 'required');
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                $unit_price = $this->erp->formatDecimal($_POST['unit_price'][$r]); 
				 $item_price = $this->erp->formatDecimal($_POST['price'][$r]);
			...
			if($_POST['paid_by'][0] == 'depreciation'){
				...
				$data['term'] = $no;
				//$this->erp->print_arrays($_POST['depreciation_term']);
			}else{
				$loans = array();
			}
			 $amount = 0;
        ...
        if ($this->form_validation->run() == true && !empty($products) && !empty($data)) {
            ...
            if ($suspend) {
				...
				for($i=0; $i<count($products);$i++){
					$products[$i]['suspend_id'] = $this->input->post('suspend_id');
				}
				*/
                ...
                //$data['suspend_id'] = $this->input->post('table_no');
				//$data['suspend_name'] = $this->input->post('suspend_name');
    ...
    function getProductDataByCode($code = NULL, $warehouse_id = NULL, $cust_id = null, $suspend_id = null, $item_rows = null, $sub_total = null)
    {
        ...
        if ($row) {
            ...
            if ($row->tax_rate) {
                ...
                $pr = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")",'image' => $row->image, 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'options' => $options, 'item_price' => $row->price,'orderqty'=>$orderqty->quantity, 'group_prices' => $group_prices, 'all_group_prices' => $all_group_prices);
            } else {
                $pr = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")",'image' => $row->image, 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => false, 'options' => $options, 'item_price' => $row->price,'orderqty'=>$orderqty->quantity, 'group_prices' => $group_prices, 'all_group_prices' => $all_group_prices);
            } 
            echo json_encode($pr);
        } else {
	...
	function add_deliveriess($start_date = NULL, $end_date = NULL)
    {
        $this->erp->checkPermissions('add_delivery',NULL,'sales');
		if (!$start_date) {
            //$start = $this->db->escape(date('Y-m') . '-1');
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('sales'), 'page' => lang('sales')), array('link' => '#', 'page' => lang('deliveries')));
        $meta = array('page_title' => lang('deliveries'), 'bc' => $bc);
        $this->load->view('default/views/pos/add_deliveriess', $this->data);
    }
