...
class Products extends MY_Controller
{
	...
	*/	
        ...
        if ($warehouse_id) {
			...
			$this->db->dbprefix('products') . ".name_kh as name_kh, " . 
			...
			$this->db->dbprefix("units").".name as unit, wp.rack as rack, alert_quantity", FALSE)		
            ->from('products');
            ...
            $this->datatables
				...
				$this->db->dbprefix('units').".name as unit, '' as rack, alert_quantity", FALSE)
                ->from('products')
	...
	function product_actions($wh = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
			...
			//			echo $pr->id;
					...
					$this->excel->getActiveSheet()->getStyle('A1'. $row.':J1'.$row)->getFont()->setBold(true);
                    ...
                    $row = 2;
						...
						foreach ($_POST['val'] as $id) {
							...
							$i = $row+1;
							$this->excel->getActiveSheet()->SetCellValue('F' . $i, $sum_cost);
							$this->excel->getActiveSheet()->SetCellValue('G' . $i, $sum_price);
            ...
            if (!empty($_POST['val'])) {
                ...
                if ($this->input->post('form_action') == 'export_excel' || $this->input->post('form_action') == 'export_pdf') {
                    ...
                    foreach ($_POST['val'] as $id) {
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, $converts->cost);
                        $this->excel->getActiveSheet()->SetCellValue('D' . $row, $this->erp->formatMoneyPurchase($converts->Quantity));
                        $this->excel->getActiveSheet()->SetCellValue('E' . $row, strip_tags($converts->noted));
                        $this->excel->getActiveSheet()->SetCellValue('F' . $row, $converts->war_name);
                        $this->excel->getActiveSheet()->SetCellValue('G' . $row, $user->first_name . ' ' . $user->last_name);
						
						$i = $row+1;
                        $this->excel->getActiveSheet()->SetCellValue('C' . $i, $sum_cost);
	...
	function get_using_stock()
    {
        ...
        $this->datatables
			...
			if($empno!=''){
			 ...
			 <a href='" . site_url('products/print_using_stock_by_id/$1/$2') . "'  class='tip' title='Print'>
			 <i class=\"fa fa-file-text-o\"></i>
			 </a> 
			 ", "id,type");
        echo $this->datatables->generate();
