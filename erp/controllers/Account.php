...
class Account extends MY_Controller
{
  ...
  //       }
    ...
    public function import_chart_csv()
    {
    	...
    	if ($this->form_validation->run() == true) {
    		...
    		if (isset($_FILES["csv_file"])) /* if($_FILES['userfile']['size'] > 0) */ {
                ...
                foreach ($final as $record) {
                    $accountcode[]      = trim($record['accountcode']);
                    $accountname[]      = trim($record['accountname']);
                    $parent_acc[]       = trim($record['parent_acc']);
                    $sectionid[]        = trim($record['sectionid']);
                    $bank[]             = trim($record['bank']);
                    $account_tax_id[]   = trim($record['account_tax_id']);
                    $acc_level[]        = trim($record['acc_level']);
                    $lineage[]          = trim($record['lineage']);
                    $data[]             = $record;
                }

            }

            $ikeys = array('accountcode','accountname','parent_acc','sectionid','bank','account_tax_id','acc_level','lineage');
            $items = array();

            foreach (array_map(null, $accountcode, $accountname, $parent_acc, $sectionid, $bank, $account_tax_id, $acc_level, $lineage) as $ikey => $value) {
    ...
    function ar_by_customer_ar()
    {
        ...
        $this->data['billers'] = $this->site->getCompanyByID($this->default_biller_id);
        $bc = array( array('link' => base_url(), 'page' => lang('home')), array('link' => '#', 'page' => lang('accounts')) );
	...
	function ar_by_customer()
	{
		...
		$cust_data[] = "";
		$customers = $this->accounts_model->ar_by_customer_detail($customer,$biller_id,$start_date, $end_date,  $balance);
    ...
    function customer_balance_ar()
    {
        ...
        $this->page_construct('reports/customer_balance_ar', $meta, $this->data);
    }
    ...
    function sale_customer_summary($pdf = NULL,$biller_id = NULL, $xls = NULL)
    {
        ...
        $this->data['warefull'] 	= $this->reports_model->getWareFullByUSER();
...
//echo $warehouse_id;exit;
        ...
        $this->data['end']= $end;
        $this->data['products_code_w']= $this->reports_model->getProductCodeByIdW($customer_id);
        $this->data['wid']= $warehouse_id;
        $this->data['type']= $type;
        $this->data['category2']= $category;
        $this->data['products_code_w']= $this->reports_model->getProductCodeByIdW($warehouse_id,$category,$customer_id,$product_id);
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('sale_by_customer_summary')));
        $meta = array('page_title' => lang('sale_by_customer_summary'), 'bc' => $bc);
        $this->page_construct('reports/sale_by_customer_summary', $meta, $this->data);
    }
    ...
    function customer_balance_r()
    {
        ...
        $cust_data[] = "";
        $customers = $this->accounts_model->ar_by_customer_collection($customer, $start_date, $end_date, $balance);
