...
class Account extends MY_Controller
{
	...
	public function getpending_Purchases($warehouse_id = null)
	{
		...
		if ($warehouse_id) {
			...
			$this->datatables
			->select("purchases.id, companies.supplier,
				SUM(
					IFNULL(grand_total, 0)
				) AS grand_total,
				SUM(
	...
	function list_ar_aging_actions(){
	$this->form_validation->set_rules('form_action', lang("form_action"), 'required');	
		if ($this->form_validation->run() == true) {
        		...
        		if ($this->input->post('form_action') == 'export_excel1'|| $this->input->post('form_action') == 'export_pdf1') {
                   ...
                   $row = 2; 
					foreach ($_POST['val'] as $id) {						
						...
						$this->excel->getActiveSheet()->SetCellValue('F' . $row, $account->date);
						$new_row = $row+1;
			...
			if (!empty($my_data)) {
				...
				foreach ($my_data as $data_row) {
					...
					foreach ($data_row['supplierDatas']['suppPO'] as $value) {
						...
						$this->excel->getActiveSheet()->SetCellValue('H' . $row, $value->order_discoun);
