	 ...
	 $(document).on('change', '#pgroup_price', function () {
		...
		if(item.all_group_price !== false) {
			$.each(item.all_group_price, function () {
				...
				if(opt ==0){
					$('#pprice,#pprice_show').val(default_price);
					...
					if(this.price_group_id == opt && this.price != 0 && this.price != '' && this.price != null) {
						...
						if(this.rate!=0)
						{
							...
							$('#pprice,#pprice_show').val(parseFloat((this.price/this.rate)*1).toFixed(2));
							$("#net_price").text(formatMoney((this.price/this.rate)*1));
						}else{
							$('#pprice,#pprice_show').val(parseFloat(this.price).toFixed(2));
							$("#net_price").text(formatMoney(this.price));
						}	
					}
				}
...
function loadItems() {
	if (localStorage.getItem('slitems')) {
		...
		$.each(slitems, function () {
			var item = this;
			//alert(JSON.stringify(item.all_group_price));
			...
			var product_id = item.row.id,
				...
				//alert(JSON.stringify(item.row.price_group_id));
				var exchange_rate = $("#exchange_rate").val();
				var is_edit = $("#is_edit").val() ? $("#is_edit").val() : 0;
				var default_price = 0;
				
		//	alert(JSON.stringify(all_group_price));
						...
						if(group_prices){
							$.each(group_prices, function(){
								...
								if(item.makeup_cost == 1){
									//cur_price_1 = $row->cost + (($row->cost * $customer_group->percent) / 100);
								}else{
					...
					if(item.row.item_load==0)
					{
						...
						if(group_prices){
							$.each(group_prices, function(){
								...
								//alert(item.customer_percent+"--"+item_item_cur);
								item_price = cur_price_1;
								unit_price = cur_price_1;
								real_unit_price = cur_price_1;
								item.row.price_id = this.price_group_id;
			...
			if (item_promotion == 1 && (current_date >= start_date && current_date <= end_date)){
				...
				// tr_html += '<td class="text-right"><input class="form-control input-sm text-right rprice" name="net_price[]" type="hidden" id="price_' + row_no + '" value="' + item_price + '"><input class="ruprice" name="unit_price[]" type="hidden" value="' + parseFloat(real_unit_price).toFixed(2) + '"><input class="realuprice" name="real_unit_price[]" type="hidden" value="' + item.row.real_unit_price + '"><span class="text-right spricesprice" id="sprice_' + row_no + '">' + formatMoney(item_price) + '</span></td>';
				tr_html += '<td class="text-right"><input class="form-control text-right rprice_t" name="net_price[]" type="text" id="price_' + row_no + '" value="' + parseFloat(real_unit_price).toFixed(2) + '"><input class="ruprice" name="unit_price[]" type="hidden" value="' + parseFloat(real_unit_price).toFixed(2) + '"><input class="realuprice" name="real_unit_price[]" type="hidden" value="' + parseFloat(real_unit_price).toFixed(2) + '"> </td>';
			}
