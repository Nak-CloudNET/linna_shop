		...
		$.each(positems, function (i, e) {
			...
			if (site.settings.tax1 == 1) {
				if (pr_tax !== false) {
					if (pr_tax.type == 1) {
						if (item_tax_method == '0') {   
							pr_tax_val = formatDecimal(parseFloat(((unit_price) * parseFloat(pr_tax.rate)) / (100 + parseFloat(pr_tax.rate)))); 		
							pr_tax_rate = parseFloat(pr_tax.rate) + '%';
						} else {
							pr_tax_val = formatDecimal(((unit_price - formatDecimal(subtotal_discount/item_qty)) * parseFloat(pr_tax.rate)) / 100);
							pr_tax_rate = parseFloat(pr_tax.rate) + '%';
						}
			...
			if (item_promotion == 1){
				tr_html += '<input class="rprice" name="net_price[]" type="hidden" id="price_' + row_no + '" value="' + real_unit_price + '"><input class="ruprice" name="unit_price[]" type="hidden" value="' + real_unit_price + '"><input class="realuprice" name="real_unit_price[]" type="hidden" value="' + real_unit_price + '"><span class="text-right sprice" id="sprice_' + row_no + '">' + formatMoney(parseFloat(real_unit_price)) + '</span></td>';
			}else{
				tr_html += '<input class="rprice" name="net_price[]" type="hidden" id="price_' + row_no + '" value="' + real_unit_price + '"><input class="ruprice" name="unit_price[]" type="hidden" value="' + real_unit_price + '"><input class="realuprice" name="real_unit_price[]" type="hidden" value="' + real_unit_price + '"><span class="text-right sprice" id="sprice_' + row_no + '">' + formatMoney(parseFloat(real_unit_price)) + '</span></td>';
			}
